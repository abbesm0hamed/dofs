...
local function formattingFunc(bufnr)
  ...
  if vim.bo[bufnr].buftype ~= "" or not vim.loop.fs_stat(bufname) or not vim.api.nvim_buf_is_valid(bufnr) then return end
  local ft = vim.bo[bufnr].filetype
  local useLsp = vim.tbl_contains(lspFormatFt, ft) and "always" or false

  if string.match(bufname, "%.env$") then return end
  if vim.tbl_contains(autoIndentFt, ft) then u.normal("gg=G``") end

  if ft == "typescript" then
    local actions = {
      "source.fixAll.ts", "source.addMissingImports.ts",
      "source.removeUnusedImports.ts", "source.organizeImports.biome",
    }
    for i = 0, #actions do
